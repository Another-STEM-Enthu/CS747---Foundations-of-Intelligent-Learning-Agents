...
if __name__ == '__main__':
  ...
  # print(regrets)
  task2p2s = np.arange(0,0.905,0.05)
  # task2p1s = [0.9 for p in range(len(task2p2s))]
  task2p1s = task2p2s + 0.1
  regrets = [0 for p in range((len(task2p1s)))]
  regrets = (task2(UCB, 30000, task2p1s, task2p2s, 50))
  print(regrets, np.mean(np.asarray(regrets)))
  plt.plot(task2p2s,regrets)
  plt.xlabel('p2')
  plt.ylabel('regret')
  # plt.title('p1 - p2 = 0.1 for KL-UCB')
  plt.show()
  pass