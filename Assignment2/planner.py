...
#=============================================================================================================================================================================================
        ...
        tpm = [[{} for p in range(N_A)] for q in range(N_S)]
        rewards = [[{} for p in range(N_A)] for q in range(N_S)]
        ...
        if len(policy_lines) > 0:
            ...
            while(delta_pe>1e-11):
                ...
                for s in range(0,N_S): 
                    sum = 0 
                    for s_ in tpm[s][A[s]].keys():
                        sum += tpm[s][A[s]][(s_)]*(rewards[s][A[s]][(s_)] + gamma*vn[(s_)])    #s_ is s'
                    V[s] = sum
        ...
        if (algo == "vi" and len(policy_lines) < 1):
            ...
            while(delta>1e-9):
                ...
                for s in range(0,N_S): 
                    ...
                    for action in range(0,N_A):
                        ...
                        for s_ in tpm[s][action].keys():
                            sum += tpm[s][action][(s_)]*(rewards[s][action][(s_)] + gamma*vn[s_])    #s_ is s'
            ...
            for s in S1:
                for a in A1:
                    problem += lpSum([tpm[s][a][(s_)] * (rewards[s][a][(s_)] + gamma * V[str(s_)]) for s_ in (tpm[s][a].keys())]) <= V[str(s)] 
            problem.solve()
            ...
            for s in range(0,N_S): 
                ...
                for action in range(0,N_A):
                    ...
                    # for s_ in range(0,N_S):
                    for s_ in tpm[s][action].keys():
                        sum += tpm[s][action][(s_)]*(rewards[s][action][(s_)] + gamma*V[s_])    #s_ is s'
                    sum_fordiff_a[action] = sum
                max_a = sum_fordiff_a.index(max(sum_fordiff_a))
                A[s] = max_a
