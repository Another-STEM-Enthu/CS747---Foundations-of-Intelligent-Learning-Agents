...
def find_p(b1,b2,r, q, possession):
    ...
    xr, yr = numbertoxy(r)
    b1r = math.sqrt((yb1 - yr)**2 + (xb1 - xr)**2)
    b2r = math.sqrt((yb2 - yr)**2 + (xb2 - xr)**2)
    b1b2 = math.sqrt((yb2 - yb1)**2 + (xb2 - xb1)**2)
    ...
    m = abs((yb1 - yr)/(xb1 - xr + 1e-7) - (yb1-yb2)/(xb1-xb2 + 1e-7))
    m_ = abs((yb1-yb2)/(xb1-xb2 + 1e-7))
    if (m < 1e-5) and (b1r + b2r <= b1b2 + 1e-2) and (abs(m_- 1)<1e-9):
        p_pass *= 0.50
        # print(b1+1,r+1,b2+1)
